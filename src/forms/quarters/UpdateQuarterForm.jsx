...
import {
  useCreateQuarterAndAddOccupants,
  useFetchQuarterDetailsByQuarterNo,
} from "../../hooks/occpantsQueries";
import { useEffect, useRef } from "react";

const UpdateQuarterForm = ({ rowState }) => {
  // Hooks
  ...
  const quarterDetailsQuery = useFetchQuarterDetailsByQuarterNo(
    rowState?.quarterNo
  );
  ...
  const createAndAddOccupantQuery = useCreateQuarterAndAddOccupants(
    (response) => {
      ...
      toast({
        ...
        title: "Success",
        description:
          response.data.detail || "Quarter and occupants updated successfully",
      });
      return response;
    },
    (error) => {
  ...
  const updateQuery = useUpdateQuarter(
    (response) => {
      queryClient.invalidateQueries({ queryKey: ["fetch-quarters-by-type"] });
      navigate("/est/quarters");
      toast({
        ...
        status: "success",
        title: "Success",
        description: response.data.detail || "Quarter updated successfully",
      });
      return response;
    },
    (error) => {
      toast({
        ...
        description:
          error.response.data.detail ||
          "Oops! Something went wrong. Couldn't update quarter.",
      });
      return error;
    }
  );

  const quarterDetails = quarterDetailsQuery?.data?.data;

  // Formik
  const initialValues = {
    quarterTypeCode: quarterDetails?.quarterTypeCode || "",
    quarterNo: quarterDetails?.quarterNo || "",
    quarterName: quarterDetails?.quarterName || "",
    physicalStatus: `${quarterDetails?.physicalStatus}`,
    quarterStatus: `${quarterDetails?.quarterStatus}`,
    departmentCode: quarterDetails?.departmentCode || "",
    officeCode: quarterDetails?.officeCode || "",
    assetType: quarterDetails?.assetType || "",
    assetDescription: quarterDetails?.assetDescription || "",
    districtCode: quarterDetails?.districtCode || "",
    blockCode: quarterDetails?.blockCode || "",
    villageCode: quarterDetails?.villageCode || "",
    location: quarterDetails?.location || "",
    latitude: quarterDetails?.latitude || "",
    longitude: quarterDetails?.longitude || "",
    landAssetId: quarterDetails?.landAssetId || "",
    assetCategory: quarterDetails?.assetCategory || "",
    structureType: quarterDetails?.structureType || "",
    inaugurationDate: quarterDetails?.inaugurationDate || "",
    builtUpArea: quarterDetails?.builtUpArea || "",
    managedBy: quarterDetails?.managedBy || "",
    // Occupants
    name: quarterDetails?.name || "",
    designation: quarterDetails?.designation || "",
    gender: quarterDetails?.gender || "",
    allotmentDate: quarterDetails?.allotmentDate || "",
    deptOffice: quarterDetails?.deptOffice || "",
    payScale: quarterDetails?.payScale || "",
    occupationDate: quarterDetails?.occupationDate || "",
    retirementDate: quarterDetails?.retirementDate || "",
  };
  ...
  const onSubmit = (values) => {
    ...
    if (
      `${formData.quarterStatus}` === "1" ||
      `${formData.quarterStatus}` === "2"
    ) {
  ...
  useEffect(() => {
    ...
    if (quarterDetailsQuery.isSuccess) {
      occupancyQuery.mutate({ quarterStatus: quarterDetails?.physicalStatus });

      if (quarterDetails?.districtCode) {
        blockQuery.mutate({ districtCode: quarterDetails?.districtCode });

        if (quarterDetails?.blockCode) {
          villageQuery.mutate({ blockCode: quarterDetails?.blockCode });
        }
      }
    }
  }, [quarterDetailsQuery.isSuccess]);
...
export default UpdateQuarterForm;
